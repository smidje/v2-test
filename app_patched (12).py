...
# === BEGIN: DUIKBREVET HELPERS + LEDEN SAVE (minimale patch) ===
ALLOWED_BREVETTEN = {
    'k1ster*', '1ster', '2ster', '3ster', '4ster', '1*I', '2*I', '3*I'
}
...
def sanitize_duikbrevet(val):
    """
    Zet lege / 'geen' waarden om naar None, trimt whitespace en mapt varianten
    naar de toegelaten set. Geeft een ValueError bij iets ongeldig.
    """
    if val is None:
        return None
    raw = str(val).strip()
    if raw == '' or raw.lower() in {'geen', 'none', '-', 'n.v.t.', 'nvt'}:
        return None

    key = raw.replace(' ', '').lower()
    if key in _DUIKBREVET_MAP:
        canon = _DUIKBREVET_MAP[key]
    else:
        canon = raw

    if canon not in ALLOWED_BREVETTEN:
        raise ValueError(
            f"Ongeldig duikbrevet: '{raw}'. Toegelaten: {sorted(ALLOWED_BREVETTEN)}"
        )
    return canon

def leden_save(supabase_url, anon_key, payload):
    """
    Minimale, veilige save:
    - normaliseert duikbrevet
    - PATCH bij update (id bekend)
    - POST bij aanmaak (zonder on_conflict)
    Geeft bij API-fout een nette Streamlit-melding en {} terug.
    """
    import httpx
    try:
        payload = dict(payload)
        payload["duikbrevet"] = sanitize_duikbrevet(payload.get("duikbrevet"))
    ...
    with httpx.Client(timeout=30.0) as client:
        if payload.get("id"):
            ...
            body = {
                ...
                "duikbrevet": payload.get("duikbrevet"),
                "actief": payload.get("actief"),
                "opt_in_weekly": payload.get("opt_in_weekly"),
            }
            body = {k: v for k, v in body.items() if k in payload and v is not None}
            resp = client.patch(
                f"{supabase_url}/rest/v1/leden?id=eq.{lid_id}",
            ...
            body = {
                ...
                "email": payload.get("email"),
                "duikbrevet": payload.get("duikbrevet"),
                "actief": payload.get("actief", True),
                "opt_in_weekly": payload.get("opt_in_weekly", True),
            }
            resp = client.post(
                f"{supabase_url}/rest/v1/leden",
                headers=headers, json=body
            )
    ...
    if resp.status_code not in (200, 201, 204):
        try:
            import streamlit as st
            st.error(f"API-fout bij leden opslaan: {resp.status_code} – {resp.text}")
        except Exception:
            pass
        return {}
    ...
    try:
        ...
        return {}
# === END: DUIKBREVET HELPERS + LEDEN SAVE ===
