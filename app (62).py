...
def page_overzicht():
    ...
    # Zorg dat we geldige datetime.date waardes hebben (NaT/None -> vandaag)
    try:
        from datetime import date as _date
        import pandas as _pd
        if _pd.isna(min_d) or _pd.isna(max_d):
            min_d = max_d = _date.today()
        else:
            min_d = min_d if isinstance(min_d, _date) else _pd.to_datetime(min_d).date()
            max_d = max_d if isinstance(max_d, _date) else _pd.to_datetime(max_d).date()
    except Exception:
        pass
