...
def duikers_labels() -> list[str]:
    res = run_db(lambda c: c.table("duikers").select("voornaam, achternaam, naam").execute(),
                 what="duikers select")
    rows = res.data or []
    # Fallback: haal actief leden met duikbrevet als er nog geen duikers zijn
    if not rows:
        try:
            ...
            _rows = _res.data or []
            rows = [{"voornaam": r.get("voornaam"), "achternaam": r.get("achternaam"), "naam": None} for r in _rows if r.get("duikbrevet") and (r.get("actief") is True or r.get("actief") is None)]
        except Exception:
            pass
    out = []
    for r in rows:
