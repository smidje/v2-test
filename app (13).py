...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        # Zorg dat de aantallen gedefinieerd zijn voor de rechterkolom
        try:
            coming_people; not_coming_people
        except NameError:
            try:
                coming_people = 0
                not_coming_people = 0
                if not s.empty:
                    for __i, __r in s.iterrows():
                        _st = str(__r.get('status') or '').lower()
                        if _st == 'yes':
                            _info = parse_meal_choice_value(__r.get('meal_choice'))
                            _guests = int((_info.get('guests') or 0))
                            if _guests < 0: _guests = 0
                            coming_people += 1 + _guests
                        elif _st == 'no':
                            not_coming_people += 1
            except Exception:
        ...
        with col_right:
            st.markdown("<div style='text-align:right; font-weight:600;'>✅ {cp} · ❌ {nc}</div>".format(cp=coming_people, nc=not_coming_people), unsafe_allow_html=True)
        with col_left:
