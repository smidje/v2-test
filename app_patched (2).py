...
import re
def is_valid_member_password(p: str) -> bool:
    if p is None:
        return False
    return bool(re.fullmatch(r'[A-Za-z0-9]{5}', str(p)))
...
def page_setup_first_admin_username():
    ...
    if submitted:
        ...
        # Validatie: wachtwoord exact 5 alfanumerieke tekens
        if not is_valid_member_password(pw1):
            st.warning("Wachtwoord moet exact 5 tekens zijn (letters of cijfers).")
            st.stop()
