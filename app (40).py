...
# === nieuw: helpers voor bewerken/verwijderen leden ===
def leden_update_by_id(lid_id: str, payload: dict):
    if is_readonly():
        raise Exception("Read-only modus")
    run_db(lambda c: c.table("leden").update(payload).eq("id", lid_id).execute(), what="leden update")

def leden_delete_by_id(lid_id: str):
    if is_readonly():
        raise Exception("Read-only modus")
    run_db(lambda c: c.table("leden").delete().eq("id", lid_id).execute(), what="leden delete")

def auth_delete_by_username(username: str):
    if is_readonly():
        raise Exception("Read-only modus")
    run_db(lambda c: c.table("auth_local").delete().eq("username", username).execute(), what="auth_local delete")
...
def signups_get(activiteit_id: str) -> pd.DataFrame:
    ...
    for col in ["id","activiteit_id","username","lid_id","status","eating","meal_choice","signup_ts", "opmerking"]:
...
def signup_upsert(activiteit_id: str, username: str | None, lid_id: str | None, status: str, eating: bool | None, meal_choice: str | None, opmerking: str | None = None):
    if is_readonly():
        raise Exception("Read-only modus")
...
def page_profiel():
    ...
    if st.button("Wijzig wachtwoord"):
        if not npw1 or len(npw1) < 5 or npw1 != npw2:
            st.warning("Min. 5 tekens en beide velden moeten gelijk zijn.")
        else:
            try:
...
def page_ledenbeheer():
    ...
    # ==== NIEUW: Bewerken/verwijderen ====
    st.divider()
    st.subheader("Lid bewerken of verwijderen")
    if df.empty:
        st.caption("Geen leden beschikbaar om te bewerken.")
    else:
        df_sorted = df.sort_values(["achternaam","voornaam"], na_position="last").reset_index(drop=True)
        labels = []
        id_map = {}
        for _, r in df_sorted.iterrows():
            achternaam = (r.get("achternaam") or "").strip()
            voornaam = (r.get("voornaam") or "").strip()
            email_lbl = (r.get("email") or "").strip()
            lbl = f"{achternaam}, {voornaam} — {email_lbl}" if (achternaam or voornaam) else (email_lbl or r.get("username") or "lid")
            labels.append(lbl)
            id_map[lbl] = r

        sel_lbl = st.selectbox("Kies een lid", labels, index=0, key="edit_lid_sel")

        if sel_lbl:
            row = id_map.get(sel_lbl, {})
            lid_id = row.get("id")
            cur_username = (row.get("username") or "").strip()

            with st.form("edit_lid_form"):
                c1, c2, c3 = st.columns(3)
                with c1:
                    evn = st.text_input("Voornaam", value=row.get("voornaam") or "")
                    ean = st.text_input("Achternaam", value=row.get("achternaam") or "")
                    eemail = st.text_input("E-mail", value=row.get("email") or "")
                with c2:
                    st.text_input("Login (username)", value=cur_username or "", disabled=True)
                    cur_role = (row.get("role") or "member")
                    idx_role = ROLE_CHOICES.index(cur_role) if cur_role in ROLE_CHOICES else 2
                    erole = st.selectbox("Rol/functie", options=ROLE_CHOICES, index=idx_role)
                with c3:
                    cur_brevet = row.get("duikbrevet")
                    if cur_brevet in BREVET_CHOICES:
                        idx_b = 1 + BREVET_CHOICES.index(cur_brevet)
                    else:
                        idx_b = 0
                    ebrevet = st.selectbox("Duikbrevet", options=["(geen)"] + BREVET_CHOICES, index=idx_b)
                    eoptin = st.toggle("Wekelijkse mail", value=bool(row.get("opt_in_weekly", True)))
                    eactief = st.toggle("Actief", value=bool(row.get("actief", True)))
                save_btn = st.form_submit_button("Bewaar wijzigingen", type="primary", disabled=is_readonly())

            if save_btn:
                if not lid_id:
                    st.error("Dit lid heeft geen ID — kan niet bewerken."); st.stop()
                eemail_eff = (eemail or "").strip().lower() or (f"{(cur_username or 'lid').strip()}@local")
                payload = {
                    "voornaam": (evn or "").strip(),
                    "achternaam": (ean or "").strip(),
                    "email": eemail_eff,
                    "role": erole,
                    "duikbrevet": None if ebrevet == "(geen)" else ebrevet,
                    "opt_in_weekly": bool(eoptin),
                    "actief": bool(eactief),
                }
                try:
                    leden_update_by_id(lid_id, payload)
                    if cur_username:
                        run_db(lambda c: c.table("auth_local").update({
                            "role": erole,
                            "email": eemail_eff or None
                        }).eq("username", cur_username).execute(), what="auth_local update (edit lid)")
                    st.success("Wijzigingen opgeslagen.")
                    st.rerun()
                except Exception as e:
                    st.error(f"Opslaan mislukt: {e}")

            with st.expander("🗑️ Lid verwijderen (gevaarlijk)"):
                colv1, colv2 = st.columns([1,1])
                with colv1:
                    also_login = st.checkbox("Verwijder ook login (auth_local)")
                with colv2:
                    confirm = st.checkbox("Ja, ik weet het zeker")
                if st.button("Verwijder dit lid", type="primary", disabled=(not confirm or is_readonly())):
                    try:
                        if not lid_id:
                            st.error("Dit lid heeft geen ID — kan niet verwijderen.")
                        else:
                            leden_delete_by_id(lid_id)
                            if also_login and cur_username:
                                auth_delete_by_username(cur_username)
        ...
        try:
            ...
            if not u:
                if not pw1 or len(pw1) < 5 or pw1 != pw2:
                    st.warning("Nieuwe login vereist wachtwoord (min. 5 tekens) en beide velden gelijk.")
                    return
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        # Rechterkolom met ✅/❌ totalen per activiteit (niet tonen bij 0)
        col_left, col_right = st.columns([8, 2])
        with col_right:
            import json as _json
            coming_count, not_count = 0, 0
            try:
                if s is not None and not s.empty:
                    for _, _r in s.iterrows():
                        _st = str(_r.get('status') or '').lower()
                        if _st == 'yes':
                            _g = 0
                            _mc = _r.get('meal_choice')
                            if isinstance(_mc, str) and _mc.strip().startswith('{'):
                                try: _g = int((_json.loads(_mc).get('guests') or 0))
                ...
                coming_count, not_count = 0, 0
            parts = []
            if coming_count > 0: parts.append(f'✅ {coming_count}')
            if not_count > 0: parts.append(f'❌ {not_count}')
            _label_counts = ' · '.join(parts)
            st.markdown(
                f"<div style='text-align:right; font-weight:600;'>{_label_counts}</div>" if _label_counts else "<div style='text-align:right;'>&nbsp;</div>",
                unsafe_allow_html=True,
            )
        with col_left:
            with st.expander(f"{titel}   ·   {badge}", expanded=False):
                ...
                if current_role() in {"admin", "user", "member"} and not is_readonly():
                    ...
                    prev_note = ""
                    if myrow is not None and not myrow.empty and ("opmerking" in myrow.columns):
                        v = myrow.iloc[0].get("opmerking")
                        if pd.notna(v):
                            prev_note = str(v)
                ...
                if "opmerking" in s.columns:
                    cols = cols[:4] + ["opmerking"] + cols[4:]
