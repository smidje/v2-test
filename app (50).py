...
def page_login_username():
    st.title("Inloggen")
    dbg = st.toggle("DB debug tonen", value=False, help="Toon DB-verbinding en auth_local status")
    if dbg:
        page_db_debug()
...
def page_db_debug():
    st.subheader("DB Debug")
    try:
        supa = st.secrets.get("supabase", {})
    except Exception:
        supa = {}
    url_s = supa.get("url")
    key_s = supa.get("anon_key")
    url_e = os.getenv("SUPABASE_URL")
    key_e = os.getenv("SUPABASE_ANON_KEY")

    eff_url = url_s or url_e or ""
    eff_key = key_s or key_e or ""
    ref = _project_ref_from_url(eff_url)

    st.write("**Effective URL:**", eff_url if eff_url else "∅")
    st.write("**Project ref (afgeleid):**", ref or "∅")
    if eff_key:
        st.write("**Anon key prefix:**", eff_key[:12] + "…")
    else:
        st.write("**Anon key:** ∅")

    st.write("**force_readonly:**", bool(st.secrets.get("app", {}).get("force_readonly", False)))

    # Probeer auth_local te lezen
    try:
        cnt = run_db(lambda c: c.table("auth_local").select("id", count="exact").execute(),
                     what="auth_local count").count or 0
        st.write("**auth_local rows (count):**", int(cnt))
    except Exception as e:
        st.error(f"Leesfout auth_local: {e}")

    # Zoek Dirk
    try:
        res = run_db(lambda c: c.table("auth_local").select("*").eq("username","Dirk").limit(1).execute(),
                     what="auth_local Dirk")
        rows = res.data or []
        if rows:
            r = rows[0]
            st.success("Gevonden gebruiker Dirk")
            st.json({k: (v if k != "password_hash" else (v[:25] + "…")) for k,v in r.items()})
        else:
            st.warning("Geen gebruiker 'Dirk' gevonden in auth_local.")
    except Exception as e:
        st.error(f"Zoekfout Dirk: {e}")
