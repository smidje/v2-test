...
def get_allow_extra(activiteit_id):
    """Lees 'allow_extra' uit activiteiten (boolean)."""
    try:
...
def set_allow_extra(activiteit_id, value: bool):
    """Bewaar 'allow_extra' per activiteit (boolean)."""
    try:
...
def page_login_username():
    ...
    if not _verify_password(pw, user.get("password_hash") or ""):
        # Fallback: eerste login met tijdelijk wachtwoord van 5 tekens (letters/cijfers)
        import re as _re
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}   ·   Komen: {coming_people}   ·   Niet komen: {not_coming_people}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                st.divider()
                # Beheerder: per activiteit extra personen toestaan (persist in DB)
                ...
                # Meerdere personen inschrijven (indien toegestaan)
                extra_personen = 0
                guest_meals = {}
                ...
                # Bepaal waarde voor meal_choice (string of JSON)
