...
# ──────────────────────────────────────────────────────────────────────────────
# ──────────────────────────────────────────────────────────────────────────────
# Seed-gebruiker (eenmalig, idempotent)
# ──────────────────────────────────────────────────────────────────────────────
def ensure_user_dirk_once():
    """Maak gebruiker 'Dirk' aan met wachtwoord 'smidje' als die nog niet bestaat.
    Rol = 'admin' als er nog geen admin is, anders 'member'.
    Foutloos en idempotent: geen effect als user al bestaat of bij read-only/tabel ontbreekt."""
    try:
        u = auth_get_user_by_username("Dirk")
        if u is None:
            role = "admin" if auth_count_admins() == 0 else "member"
            auth_create_user("Dirk", "smidje", role)
    except Exception:
        # Stil falen om UI te beschermen (bijv. in read-only of ontbrekende tabellen)
        pass
...
def main():
    ...
    # Seed local user 'Dirk' (eenmalig, idempotent)
    ensure_user_dirk_once()
