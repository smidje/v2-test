...
def page_activiteiten():
    ...
    if current_role() == "admin":
        with st.expander("➕ Nieuwe activiteit"):
            ...
            with c2:
                datum = st.date_input("Datum*", value=datetime.date.today(), key="act_datum")
...
def page_duiken():
    ...
    datum = st.date_input("Datum", datetime.date.today(), key="duik_datum", format="DD/MM/YYYY")
...
def page_overzicht():
    ...
    rng = c1.date_input("Periode", (min_d, max_d), key="overz_periode")
...
def page_afrekening():
    ...
    min_d, max_d = df["Datum"].min(), df["Datum"].max()
    rng = c1.date_input("Periode", (min_d, max_d), key="afr_periode")
