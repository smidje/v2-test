...
def _count_people_from_signups(s):
    """Count YES including guests from meal_choice JSON, and NO."""
    coming, not_coming = 0, 0
    try:
        ...
        if s is not None and hasattr(s, "empty") and not s.empty:
            for _, r in s.iterrows():
                ...
                if stt == "yes":
                    guests = 0
                    mc = r.get("meal_choice")
                    if isinstance(mc, str) and mc.strip().startswith("{"):
                        try:
                            guests = int((_json.loads(mc).get("guests") or 0))
                        except Exception:
                            guests = 0
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        # Tellingen (incl. extra personen)

        coming_count, not_count = _count_people_from_signups(s)

        _parts = []

        if coming_count > 0: _parts.append(f"✅ {coming_count}")

        if not_count > 0: _parts.append(f"❌ {not_count}")

        _label_counts = " · ".join(_parts)
        # mijn huidige status
        myrow = None
        if my_username:
...
with col_right:
    parts = []
    if int(coming_count) > 0:
        parts.append(f"✅ {int(coming_count)}")
    if int(not_count) > 0:
        parts.append(f"❌ {int(not_count)}")
    label = " · ".join(parts)
    st.markdown(
        f"<div style='text-align:right; font-weight:600;'>{label}</div>" if label else "<div style='text-align:right;'>&nbsp;</div>",
        unsafe_allow_html=True,
    )
with col_left:
    with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                ...
                # Opmerking (optioneel)
                prev_note = ""
                if myrow is not None and not myrow.empty and pd.notna(myrow.iloc[0].get("opmerking")):
                    prev_note = str(myrow.iloc[0].get("opmerking") or "")
                ...
                if myrow is not None and not myrow.empty and pd.notna(myrow.iloc[0].get("opmerking")):
                    prev_note = str(myrow.iloc[0].get("opmerking") or "")
                opmerking = st.text_area("Opmerking (optioneel)", value=prev_note, max_chars=500, key=f"note_{row['id']}") if mc == "— kies —" else mc
