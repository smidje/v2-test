...
def canon_brevet(v):
    """
    Retourneer een canonieke waarde voor duikbrevet of None.
    Alleen waarden in BREVET_CHOICES worden doorgegeven; alle andere waarden -> None.
    """
    if v is None:
        return None
    try:
    ...
    if s is None:
        return None
    s = str(s).strip().lower()
    # Extra mapping indien nodig (normalize_brevet dekt de meeste gevallen)
    extra_map = {
        "assinst": "ass-inst",
        "1inst": "1*inst",
        "2inst": "2*inst",
...
def normalize_brevet(v: str | None) -> str | None:
    """
    Normaliseer vrije tekst tot een herkenbare token. Geeft None terug voor
    lege/onbekende waarden. Deze functie probeert veel voorkomende varianten te matchen.
    """
    if v is None:
        return None
    s = str(v).strip().lower()
    ...
    s = s.replace("assistent", "ass")
    s = s.replace("ass.-", "ass-").replace("ass_", "ass-").replace("ass ", "ass-")
    s = s.replace("_", "").replace(" ", "")
    # Mapping van bekende vormen naar canonical token
    if s in {"k1", "k1ster", "kand1", "kand1ster", "kandidaat1ster"}:
        return "k1ster"
    if s in {"1", "1ster", "1*"}:
        return "1ster"
    if s in {"2", "2ster", "2*"}:
    ...
    if s in {"4", "4ster", "4*"}:
        return "4ster"
    if s in {"assinst", "ass-inst", "assinst.", "assinstr"}:
        return "ass-inst"
    if s in {"1inst", "1*inst", "1-inst", "1instructeur", "1*instructeur"}:
        return "1*inst"
    if s in {"2inst", "2*inst", "2-inst", "2instructeur", "2*instructeur"}:
        return "2*inst"
    if s in {"3inst", "3*inst", "3-inst", "3instructeur", "3*instructeur"}:
        return "3*inst"
    return s

def leden_upsert(payload: dict):
    """
    Upsert naar leden tabel, met saneer-stappen:
    - duikbrevet: alleen geldige waarden of None
    - email: leeg -> None, en lowercase
    """
    if is_readonly():
        raise Exception("Read-only modus")
    p = dict(payload)  # copy om original untouched te laten
    # Sanitize duikbrevet
    if "duikbrevet" in p:
        ...
        if val is None:
            ...
            p["duikbrevet"] = canon_brevet(val)
    # Sanitize email: lege string -> None, en lower-case
    if "email" in p:
        em = (p.get("email") or "").strip()
        p["email"] = em.lower() if em else None
    run_db(lambda c: c.table("leden").upsert(p, on_conflict="email").execute(),
           what="leden upsert")
...
def page_ledenbeheer():
    ...
    # ─────────────────────────────────────────────────────────
    st.divider()
    st.subheader("Lid bewerken / verwijderen")
    ...
    if 'df' in locals() and df is not None and not df.empty:
        ...
        for _, r in df_show.iterrows():
            ...
            meta.append({"id": rid, "email": em, "username": un})
        ...
        if sel:
            ...
            with st.form("edit_lid_form", clear_on_submit=False):
                ...
                with c1:
                    ...
                    role = st.selectbox(
                        "Rol",
                        ["admin", "user", "member", "viewer"],
                        index=["admin", "user", "member", "viewer"].index((row.get("role") or "member"))
                    )
                with c2:
                    ...
                    brevet = st.selectbox(
                        "Duikbrevet",
                        _choices,
                        index=_choices.index(cur_brevet) if cur_brevet in _choices else 0
                    )
    ...
    if current_role() == "admin":
        with st.expander("➕ Nieuwe activiteit"):
            ...
            with c2:
                ...
                tijd = tijd_tp  # gebruik Ter plaatse als hoofd-tijd
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                ...
                prev_remark = None
                if myrow is not None and not myrow.empty:
                    pr = myrow.iloc[0].get("opmerking")
                    prev_remark = pr if isinstance(pr, str) else None
                ...
                if st.button("Bewaar mijn keuze", key=f"save_{row['id']}", type="primary"):
                    ...
                    try:
                        signup_upsert(
                            ...
                            opmerking=(remark_val if status == "Ik kom" else None)
                        )
            ...
            print_df = s[["username", "status", "eating", "meal_choice", "opmerking", "signup_ts"]].copy()
...
def page_duiken():
    ...
    if not labels:
        try:
            ...
            _leden_df = pd.DataFrame(_leden_df.data or [])
            _labels_tmp = []
            if not _leden_df.empty:
                _f = _leden_df.copy()
                _f["akt"] = _f.get("actief", True).fillna(True)
                _f["brev"] = _f.get("duikbrevet").fillna("")
                _f = _f[(_f["akt"] == True) & (_f["brev"] != "") & (_f["brev"] != "(geen)")]
                for _, _r in _f.iterrows():
                    _vn = (_r.get("voornaam") or "").strip()
                    _an = (_r.get("achternaam") or "").strip()
                    _lab = f"{_an}, {_vn}".strip(", ")
                    if _lab:
                        _labels_tmp.append(_lab)
    ...
    # Nieuw: overnemen uit activiteit (vraagt enkel minimale wijzigingen)
    st.subheader("Overnemen uit activiteit")
    try:
        _acts = activiteiten_list_df(upcoming=False)
    except Exception:
        _acts = pd.DataFrame([])
    _act_opts = []
    _act_map = {}
    if not _acts.empty:
        ...
        for _, _r in _acts_sorted.iterrows():
            try:
                _d = pd.to_datetime(_r["datum"]).strftime('%d/%m/%Y')
            except Exception:
                _d = str(_r.get("datum") or "")
            _loc = (_r.get("locatie") or "").strip()
            _label = f"{_d} · {_r.get('titel') or ''}" + (f" · {_loc}" if _loc else "")
            _act_opts.append(_label)
            _act_map[_label] = _r.to_dict()
    ...
    if _sel_act != "— kies activiteit —":
        ...
        try:
            _prefill_date = pd.to_datetime(_ar.get("datum")).date() if _ar.get("datum") else None
        except Exception:
            _prefill_date = None
        try:
            _s = signups_get(_ar["id"])
        except Exception:
            _s = pd.DataFrame([])
        if not _s.empty:
            ...
            if not _s_yes.empty:
                ...
                for __, _sr in _s_yes.iterrows():
                    _vn = _an = ""
                    _un = _sr.get("username")
                    _lid = _sr.get("lid_id")
                    if _un:
                        _mem = leden_get_by_username(_un)
                        if _mem:
                            _vn = (_mem.get("voornaam") or "").strip()
                            _an = (_mem.get("achternaam") or "").strip()
                    if (not _vn and not _an) and _lid:
                        # lookup leden by id
                        try:
                            _res = run_db(lambda c: c.table("leden").select("voornaam, achternaam").eq("id", _lid).limit(1).execute(),
                                          what="leden by id")
                            _mm = _res.data or []
                            if _mm:
                                _vn = (_mm[0].get("voornaam") or "").strip()
                                _an = (_mm[0].get("achternaam") or "").strip()
                        except Exception:
                            pass
                    if _vn or _an:
                        _lab = f"{_an}, {_vn}".strip(", ")
                        if _lab in _labels_set:
                            _pref.append(_lab)
                _prefill_duikers = sorted(set(_pref))
    datum_default = _prefill_date or datetime.date.today()
    datum = st.date_input("Datum", datum_default)
    duikcode = st.text_input("Duikcode (optioneel)")
    _place_options = ["— kies —"] + plaatsen
    _place_index = 0
    if _prefill_place and _prefill_place in plaatsen:
        _place_index = 1 + plaatsen.index(_prefill_place)
    plaats = st.selectbox("Duikplaats", _place_options, index=_place_index)
    sel_duikers = st.multiselect("Duikers", labels, default=_prefill_duikers)
...
def page_overzicht():
    ...
    # Zorg voor geldige date-objecten voor de periode-selector
    try:
        import pandas as _pd
        from datetime import date as _date
        _min = _pd.to_datetime(min_d, dayfirst=True, errors='coerce')
        _max = _pd.to_datetime(max_d, dayfirst=True, errors='coerce')
        if _pd.isna(_min) or _pd.isna(_max):
            min_d = max_d = _date.today()
        else:
            min_d = _min.date() if hasattr(_min, 'date') else _date.today()
            max_d = _max.date() if hasattr(_max, 'date') else _date.today()
    except Exception:
        pass
