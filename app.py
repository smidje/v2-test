...
def duikers_labels() -> list[str]:
    """Geeft labels 'Achternaam, Voornaam' terug.
    Eerst uit tabel 'duikers'; als leeg, fallback op 'leden' met duikbrevet en (actief is True of None).
    """
    try:
...
def page_overzicht():
    ...
    view["Datum"] = pd.to_datetime(view["Datum"]).dt.strftime("%d/%m/%Y")
    view['Datum'] = view['Datum'].dt.strftime(DATE_FMT)
    st.dataframe(view, use_container_width=True, hide_index=True)
