...
def signup_upsert(activiteit_id: str, username: str | None, lid_id: str | None,
                  status: str, eating: bool | None, meal_choice: str | None, opmerking: str | None):

    """Maak of update een inschrijving voor (username of lid_id) op een activiteit."""
    if is_readonly():
        raise Exception("Read-only modus")
    ...
    if rows:
        sid = rows[0]["id"]
        return run_db(lambda c: c.table("activity_signups").update(payload).eq("id", sid).execute(),
                      what="signups update")
    else:
        return run_db(lambda c: c.table("activity_signups").insert(payload).execute(),
                      what="signups insert")

def get_allow_extra(activiteit_id):
    """Lees per-activiteit de instelling 'allow_extra' uit de activiteiten-tabel."""
    try:
        res = run_db(lambda c: c.table("activiteiten").select("allow_extra").eq("id", activiteit_id).limit(1).execute(),
                     what="activiteiten allow_extra get")
        rows = res.data or []
        if rows:
            return rows[0].get("allow_extra")
    except Exception:
        return None
    return None

def set_allow_extra(activiteit_id, value: bool):
    """Bewaar per-activiteit de instelling 'allow_extra' (boolean)."""
    try:
        run_db(lambda c: c.table("activiteiten").update({"allow_extra": bool(value)}).eq("id", activiteit_id).execute(),
               what="activiteiten allow_extra set")
        return True
    except Exception:
        return False
def appbar(tag: str):
...
def page_login_username():
    ...
    if not user:
        ...
        return
    if not _verify_password(pw, user.get("password_hash") or ""):
        # Eerste login fallback: 5 tekens (letters/cijfers) zetten als wachtwoord indien nog leeg
        import re as _re
        if (user.get("password_hash") in (None, "")) and _re.fullmatch(r"[A-Za-z0-9]{5}", str(pw) or ""):
            try:
                new_hash = _hash_password(pw)
                run_db(lambda c: c.table("auth_local").update({"password_hash": new_hash}).eq("id", user["id"]).execute(), what="auth_local set password")
                user = auth_get_user_by_username(login)
            except Exception:
                st.error("Kon wachtwoord niet initialiseren.")
                return
        else:
            st.error("Onjuiste login.")
            return
...
def parse_meal_choice_value(raw):
    """Parse meal_choice (string of JSON). Returns dict with primary, guests, guest_meals."""
    ...
    try:
        if isinstance(raw, str) and raw.strip().startswith("{"):
            ...
            guest_meals = data.get("guest_meals") or {}
            guest_meals = {str(k): int(v) for k, v in guest_meals.items() if v}
            return {"primary": primary, "guests": guests, "guest_meals": guest_meals}
    except Exception:
        pass
    if isinstance(raw, str) and raw.strip():
        return {"primary": raw.strip(), "guests": 0, "guest_meals": {}}
    return {"primary": None, "guests": 0, "guest_meals": {}}

def party_size_from_signup(row):
    status = str(row.get("status") or "").lower()
    if status != "yes":
        return 0
    info = parse_meal_choice_value(row.get("meal_choice"))
    guests = int(info.get("guests") or 0)
    if guests < 0:
        guests = 0
    return 1 + guests
...
def signups_get(activiteit_id: str) -> pd.DataFrame:
    """Haal inschrijvingen op voor een activiteit."""
    res = run_db(lambda c: c.table("activity_signups")
                           .select("*")
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        # Aantallen (voor titel)
        try:
            coming_people = 0
            if not s.empty:
                for __i, __r in s.iterrows():
                    coming_people += party_size_from_signup(__r)
            not_coming_people = int((s["status"] == "no").sum()) if "status" in s else 0
        except Exception:
            coming_people, not_coming_people = 0, 0
        # mijn huidige status
        myrow = None
        if my_username:
        ...
        with col_left.expander(f"{titel}   ·   {badge}", expanded=False):


        with col_right:


            st.markdown(f"<div style='text-align:right; font-weight:600;'>✅ {coming_people} · ❌ {not_coming_people}</div>", unsafe_allow_html=True)
if row.get("locatie"):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                st.divider()
                # Beheerder: per activiteit extra personen toestaan (persist in DB, default AAN bij maaltijdopties)
                enable_key = f"enable_extra_{row['id']}"
                try:
                    _db_allow = get_allow_extra(row["id"])
                except Exception:
                    _db_allow = None
                meal_opts = row.get("meal_options") or []
                _default_on = bool(meal_opts) and len(meal_opts) > 0
                if enable_key not in st.session_state:
                    if _db_allow is not None:
                        st.session_state[enable_key] = bool(_db_allow)
                    else:
                        st.session_state[enable_key] = bool(_default_on)
                if current_role() == "admin":
                    st.checkbox("Extra personen toestaan (beheerder)", key=enable_key, value=bool(st.session_state.get(enable_key, False)))
                    if st.button("Instelling opslaan", key=f"save_extra_{row['id']}"):
                        if set_allow_extra(row["id"], bool(st.session_state.get(enable_key, False))):
                            st.success("Instelling bewaard voor deze activiteit.")
                        else:
                            st.warning("Kon instelling niet bewaren. Bestaat kolom allow_extra op activiteiten?")
                multi_enabled = bool(st.session_state.get(enable_key, False))
                ...
                # Bestaande opmerking ophalen
                prev_note = ""
                try:
                    if myrow is not None and not myrow.empty:
                        val = myrow.iloc[0].get("opmerking") or myrow.iloc[0].get("comment")
                        if pd.notna(val):
                            prev_note = str(val)
                except Exception:
                    pass
                ...
                # Opmerking invoer
                opmerking = st.text_area("Opmerking (optioneel)", value=prev_note, max_chars=500, key=f"note_{row['id']}")
                ...
                # Meerdere personen (indien toegestaan)
                extra_personen = 0
                guest_meals = {}
                if multi_enabled and status == "Ik kom":
                    extra_personen = st.number_input("Extra personen", min_value=0, max_value=50, step=1, value=0, key=f"extra_{row['id']}")
                    if eating and meal_opts:
                        st.caption("Verdeel maaltijden voor extra personen")
                        _dist = {}
                        for i_opt, opt in enumerate(meal_opts):
                            _dist[opt] = st.number_input(f"Extra: {opt}", min_value=0, max_value=100, step=1, key=f"g_{row['id']}_{i_opt}")
                        guest_meals = _dist
                        if sum(guest_meals.values()) != int(extra_personen or 0):
                            st.warning("Let op: som van extra maaltijden komt niet overeen met extra personen.")

                # meal_choice als string of JSON (met extra personen)
                meal_choice_to_store = meal_choice
                if multi_enabled and status == "Ik kom":
                    try:
                        import json as _json
                        data_mc = {"primary": meal_choice, "guests": int(extra_personen or 0)}
                        if guest_meals:
                            data_mc["guest_meals"] = {k: int(v) for k, v in guest_meals.items() if v}
                        meal_choice_to_store = _json.dumps(data_mc, ensure_ascii=False)
                    except Exception:
                        meal_choice_to_store = meal_choice
                ...
                if st.button("Bewaar mijn keuze", key=f"save_{row['id']}", type="primary"):
                    try:
                        signup_upsert(
                            ...
                            eating=eating,
                            meal_choice=meal_choice_to_store
                        )
