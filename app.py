...
def page_ledenbeheer():
    ...
    if 'df' in locals() and df is not None and not df.empty:
        # Maak klikbare labels en metadata
        options = []
        meta = []
        ...
        sel = st.selectbox("Kies een lid", ["— kies —"] + options, index=0, key="leden_edit_sel")
        if sel != "— kies —":
            i = options.index(sel)
            m_ = meta[i]
...
def page_duiken():
    ...
    datum = st.date_input("Datum", datetime.date.today(), format="DD/MM/YYYY")
...
def page_overzicht():
    ...
    # Bepaal veilige grenzen voor de periode-selector (strings -> datetime)
    _ser = pd.to_datetime(df['Datum'], dayfirst=True, errors='coerce')
    from datetime import date as _date
    if _ser.dropna().empty:
...
# ──────────────────────────────────────────────────────────────────────────────
def duikers_labels() -> list[str]:
    """Labels 'Achternaam, Voornaam' voor selecties bij Duiken.
    1) eerst uit tabel 'duikers'; 2) als leeg: actieve leden met duikbrevet;
    3) als dat ook leeg is: alle actieve leden.
    """
    # 1) duikers
    try:
        res = run_db(lambda c: c.table("duikers").select("voornaam, achternaam, naam").execute(),
                     what="duikers select (labels)")
        rows = res.data or []
    except Exception:
        rows = []
    ...
    try:
        for r in rows:
            vn = (r or {}).get("voornaam") or ""
            an = (r or {}).get("achternaam") or ""
            nm = (r or {}).get("naam")
            if nm:
                out.append(str(nm).strip())
            else:
                lab = f"{an}, {vn}".strip(", ").strip()
                if lab:
                    out.append(lab)
    except Exception:
        pass

    # 2) fallback: leden met duikbrevet (actief True of None)
    if not out:
        try:
            r2 = run_db(lambda c: c.table("leden").select("voornaam, achternaam, duikbrevet, actief").execute(),
                        what="leden select (duikers fallback)")
            rows2 = r2.data or []
            for r in rows2:
                if r.get("duikbrevet") and (r.get("actief") is True or r.get("actief") is None):
                    vn = (r or {}).get("voornaam") or ""
                    an = (r or {}).get("achternaam") or ""
                    lab = f"{an}, {vn}".strip(", ").strip()
                    if lab:
    ...
    # 3) SECONDARY fallback: alle actieve leden
    if not out:
        try:
            r3 = run_db(lambda c: c.table("leden").select("voornaam, achternaam, actief").execute(),
                        what="leden select (duikers secondary)")
            rows3 = r3.data or []
            for r in rows3:
                if (r.get("actief") is True) or (r.get("actief") is None):
                    vn = (r or {}).get("voornaam") or ""
                    an = (r or {}).get("achternaam") or ""
                    lab = f"{an}, {vn}".strip(", ").strip()
                    if lab:
    ...
    def key(x: str) -> tuple:
        s = (x or "").split(",")
        last = s[0].strip() if s else ""
        first = s[1].strip() if len(s) > 1 else ""
