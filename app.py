...
def leden_update_by_id(lid_id: str, patch: dict):
    """
    Update velden van een bestaande rij in 'leden' op basis van id.
    Geeft (ok, data_of_fout) terug.
    """
    if is_readonly():
        return False, "Database staat op READONLY."
    def _op(sb):
        q = sb.table("leden").update(patch).eq("id", lid_id).select("*")
        resp = q.execute()
...
def leden_delete_by_id(lid_id: str):
    """
    Verwijder een lid uit 'leden' op basis van id.
    Geeft (ok, info_of_fout) terug.
    """
    if is_readonly():
        return False, "Database staat op READONLY."
    def _op(sb):
        # delete zonder select geeft geen data terug; dat is oké
        resp = sb.table("leden").delete().eq("id", lid_id).execute()
        # PostgREST geeft geen count tenzij expliciet gevraagd; we volstaan met ok
...
def page_ledenbeheer():
    ...
    st.divider()
# [AUTO-INSERT] edit/delete panel added

    # --- Bewerken / verwijderen bestaande leden ---
    st.markdown("#### Lid bewerken of verwijderen")
    _ok_df, leden_df = leden_get_df()
    if _ok_df and not leden_df.empty:
        # Gebruik label "Achternaam Voornaam – email|username"
        display_cols = ["voornaam","achternaam","email","username","duikbrevet","actief","opt_in_weekly","id"]
        for c in display_cols:
            if c not in leden_df.columns:
                leden_df[c] = None
        leden_df["_label"] = (
            leden_df["achternaam"].fillna("") + " " +
            leden_df["voornaam"].fillna("") + " – " +
            leden_df["email"].fillna("").replace("", leden_df["username"].fillna(""))
        ).str.strip(" –")
        keuze = st.selectbox(
            "Kies een lid",
            options=leden_df["_label"].tolist(),
            index=0 if len(leden_df)==0 else 0
        )
        sel = leden_df[leden_df["_label"] == keuze].iloc[0] if len(leden_df)>0 else None
        if sel is not None:
            with st.form("frm_edit_lid", enter_to_submit=False, border=True):
                col1, col2 = st.columns(2)
                with col1:
                    v_voornaam = st.text_input("Voornaam", value=sel.get("voornaam") or "")
                    v_achternaam = st.text_input("Achternaam", value=sel.get("achternaam") or "")
                    v_email = st.text_input("E-mail", value=sel.get("email") or "")
                    v_username = st.text_input("Username", value=sel.get("username") or "")
                with col2:
                    dkeuze = ["(geen)"] + BREVET_CHOICES
                    try:
                        idx = dkeuze.index(sel.get("duikbrevet")) if sel.get("duikbrevet") in dkeuze else 0
                    except ValueError:
                        idx = 0
                    v_brevet = st.selectbox("Duikbrevet", options=dkeuze, index=idx)
                    v_actief = st.checkbox("Actief", value=bool(sel.get("actief")))
                    v_weekly = st.checkbox("Opt-in weekly", value=bool(sel.get("opt_in_weekly")))
                b_save, b_del = st.columns(2)
                do_save = b_save.form_submit_button("Opslaan", use_container_width=True)
                do_del = b_del.form_submit_button("Verwijderen", use_container_width=True)
                st.caption(f"ID: {sel.get('id')}")
            if do_save:
                patch = {
                    "voornaam": v_voornaam.strip() or None,
                    "achternaam": v_achternaam.strip() or None,
                    "email": (v_email.strip() or None),
                    "username": v_username.strip() or None,
                    "duikbrevet": None if v_brevet == "(geen)" else v_brevet,
                    "actief": bool(v_actief),
                    "opt_in_weekly": bool(v_weekly),
                }
                ok_edit, data_edit = leden_update_by_id(sel.get("id"), patch)
                if ok_edit:
                    st.success("Lid bijgewerkt.")
                    st.rerun()
                else:
                    st.error(f"Bijwerken mislukt: {data_edit}")
            if do_del:
                with st.popover("Bevestig verwijderen", use_container_width=True):
                    st.warning("Deze actie kan niet ongedaan gemaakt worden.")
                    confirm = st.checkbox("Ik bevestig het verwijderen", value=False)
                    if st.button("Verwijder definitief", type="primary", disabled=not confirm):
                        ok_del, data_del = leden_delete_by_id(sel.get("id"))
                        if ok_del:
                            st.success("Lid verwijderd.")
                            st.rerun()
                        else:
                            st.error(f"Verwijderen mislukt: {data_del}")
    else:
