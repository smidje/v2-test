    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            if current_role() in {"admin", "user", "member"} and not is_readonly():
                st.divider()
            # Beheerder kan per activiteit extra personen toestaan
            enable_key = f"enable_extra_{row['id']}"
            if current_role() == "admin":
                _cur = st.session_state.get(enable_key, False)
                st.checkbox("Extra personen toestaan (beheerder)", key=enable_key, value=_cur)
