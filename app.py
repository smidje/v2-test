...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        if row.get("tijd"):
            titel += f" · {row['tijd']}"        # Aantallen per activiteit (rechts tonen; 0 verbergen)
        coming_count = int((s['status'] == 'yes').sum()) if not s.empty else 0
        not_count = int((s['status'] == 'no').sum()) if not s.empty else 0
        col_left, col_right = st.columns([8, 2])
        with col_right:
            parts = []
            if coming_count > 0:
                parts.append(f"✅ {coming_count}")
            if not_count > 0:
                parts.append(f"❌ {not_count}")
            label = " · ".join(parts)
            if not label:
                st.markdown("<div style='text-align:right;'>&nbsp;</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='text-align:right; font-weight:600;'>{label}</div>", unsafe_allow_html=True)
        with col_left:
