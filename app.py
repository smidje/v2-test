...
def leden_upsert(payload: dict):
    # Alleen deze functie aangepast: eerst normale upsert met on_conflict="email".
    # Als de DB geen unieke constraint op email heeft (42P10), val terug op handmatige upsert (select->update/insert).
    if is_readonly():
        raise Exception("Read-only modus")

    try:
        return run_db(
            lambda c: c.table("leden").upsert(payload, on_conflict="email").execute(),
            what="leden upsert"
        )
    except Exception as e:
        msg = str(e)
        if "42P10" in msg or "no unique or exclusion constraint matching" in msg:
            # Fallback: handmatig upsert op basis van email
            def _fallback(c):
                email = payload.get("email")
                if not email:
                    # zonder email geen fallback mogelijk -> opnieuw raise
                    raise
                sel = c.table("leden").select("id").eq("email", email).limit(1).execute()
                if getattr(sel, "data", None):
                    lid_id = sel.data[0]["id"]
                    return c.table("leden").update(payload).eq("id", lid_id).execute()
                else:
                    return c.table("leden").insert(payload).execute()
            return run_db(_fallback, what="leden manual upsert(email)")
        # Andere fout: opnieuw raise, zodat bestaande foutafhandeling/alerts blijven werken
        raise
