# app.py — ANWW Duikapp (lokale login + ledenbeheer door admin)
# Login via eigen tabel 'auth_local' (niet via Supabase Auth)
# Build: v2025-10-03-LOCAL-AUTH-01
...
APP_BUILD = "v2025-10-03-LOCAL-AUTH-01"
...
# ──────────────────────────────────────────────────────────────────────────────
# Lokale AUTH (auth_local): e-mail + wachtwoordhash + role
# ──────────────────────────────────────────────────────────────────────────────
...
def auth_count_admins() -> int:
    ...
    # postgrest v11: res.count may be None; fallback on length filter
...
def current_username() -> str:
    # voorkeursvolgorde: auth_local.username -> leden.username
    u = current_user()
    if u.get("username"):
...
# ──────────────────────────────────────────────────────────────────────────────
# Leden / Duikers / Activiteiten / Duiken / Afrekening helpers
# ── NB: Voor eenvoud verwacht dit dat RLS anon toelaat (of dat je RLS uit hebt).
# ──────────────────────────────────────────────────────────────────────────────
...
def duikers_list_labels() -> list[str]:
...
def page_login():
    ...
    if not user or not _verify_password(pw, user.get("password_hash") or ""):
        ...
        return
    # haal optioneel leden.username op als auth_local None heeft
...
def page_ledenbeheer():
    ...
    if submitted:
        ...
        try:
            ...
            # 2) auth_local: maak login als die nog niet bestaat
            ...
            if not u:
                ...
                # update rol / username ook in auth_local
                if not is_readonly():
...
def page_overzicht():
    ...
    if st.button("Verwijder geselecteerde", disabled=(len(sel)==0 or is_readonly())):
...
def page_beheer():
    ...
    with tabs[2]:
        ...
        np = st.text_input("Nieuwe duikplaats")
        if st.button("Toevoegen", disabled=is_readonly()):
...
def main():
    ...
    try:
        if auth_count_admins() == 0 and "auth_user" not in st.session_state:
            page_setup_first_admin()
            return
    except Exception:
        # als auth_local nog niet bestaat → toon melding
        st.error("Tabel 'auth_local' ontbreekt. Maak deze aan met SQL (zie onderaan).")
        st.stop()
    ...
    role = current_role()
    # Activiteiten eerst, zoals gevraagd
