...
def fetch_duiken(start: datetime.date = None, end: datetime.date = None) -> pd.DataFrame:
    # Leest uit je bestaande 'duiken' tabel (duiker=string)
    q = sb.table("duiken").select("*")
    if start: q = q.gte("datum", start.isoformat())
    if end:   q = q.lte("datum", end.isoformat())
    res = q.execute()
...
def compute_afrekening(df_duiken: pd.DataFrame, bedrag_per_duik: float,
    ...
    """
    Verwacht df_duiken met kolommen: Duiker, (optioneel filters al toegepast)
    Returnt per duiker: aantal_duiken, bedrag_huidig, rest_in, totaal, settled_blokken, rest_out, blokken_json
    """
    if df_duiken.empty:
...
with tabs[0]:
    ...
    c1, c2 = st.columns(2)
    with c1: v = st.text_input("Voornaam")
    with c2: a = st.text_input("Achternaam")
...
with tabs[1]:
    ...
    start, end = rng if isinstance(rng, tuple) else (default_start, today)
    # Duiken ophalen uit OUDE tabel
    ...
    # Laat handmatige correctie toe (bv iemand heeft speciaal restbedrag)
    # We maken een default tabeltje om te editen
    ...
    # Bouw een dict met overrides (leeg = gebruik DB)
        ...
        if st.button("✅ Markeer als betaald & opslaan (historiek + saldo bijwerken)"):
            try:
                for _, r in calc.iterrows():
                    ...
                    # Saldo bijwerken = nieuwe rest_out
