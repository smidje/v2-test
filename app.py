...
# Tabellen & helpers
# ──────────────────────────────────────────────────────────────────────────────
BREVET_CHOICES = ['k1ster','1ster','2ster','3ster','4ster','ass-inst','1*I','2*I','3*I']
...
def page_ledenbeheer():
    ...
    if not df.empty:
        # Maak klikbare lijstlabels en map naar ID (fallback op email/username)
        options = []
        id_map = {}
        key_map = {}  # fallback key if id ontbreekt
        ...
        for _, r in df.iterrows():
            ...
            label = f"{an}, {vn}" if (an or vn) else (em or un or "(onbekend)")
            # Voeg extra info toe voor duidelijkheid
            info = em or un
            if info:
                label += f" · {info}"
            options.append(label)
            # map to id if present else to unique email or username
            rid = r.get("id")
            id_map[label] = rid
            key_map[label] = {"email": em, "username": un}

        sel = st.selectbox("Klik/Selecteer een lid", ["— kies —"] + options, index=0)
        if sel and sel != "— kies —":
            # Zoek de gekozen rij
            rid = id_map.get(sel)
            chosen = None
            if rid is not None and "id" in df.columns:
                tmp = df.loc[df["id"] == rid]
                if not tmp.empty:
                    chosen = tmp.iloc[0].to_dict()
            if chosen is None:
                # fallback op email / username
                km = key_map.get(sel, {})
                em = km.get("email"); un = km.get("username")
                if em and "email" in df.columns:
                    tmp = df.loc[df["email"] == em]
                    if not tmp.empty: chosen = tmp.iloc[0].to_dict()
                if chosen is None and un and "username" in df.columns:
                    tmp = df.loc[df["username"] == un]
                    if not tmp.empty: chosen = tmp.iloc[0].to_dict()
            ...
            if chosen:
                with st.form("edit_lid_form"):
                    c1, c2 = st.columns(2)
                    with c1:
                        vn = st.text_input("Voornaam", value=(chosen.get("voornaam") or ""))
                        em = st.text_input("E-mail", value=(chosen.get("email") or ""))
                        role = st.selectbox("Rol", ["admin","user","member","viewer"], index=["admin","user","member","viewer"].index((chosen.get("role") or "member")))
                    with c2:
                        an = st.text_input("Achternaam", value=(chosen.get("achternaam") or ""))
                        un = st.text_input("Login (username)", value=(chosen.get("username") or ""))
                        brevet = st.selectbox("Duikbrevet", ["(geen)"] + BREVET_CHOICES, index=(["(geen)"] + BREVET_CHOICES).index((chosen.get("duikbrevet") or "(geen)")) if (chosen.get("duikbrevet") or "(geen)") in (["(geen)"] + BREVET_CHOICES) else 0)
                    optin = st.checkbox("Wekelijkse mail", value=bool(chosen.get("opt_in_weekly")))
                    actief = st.checkbox("Actief", value=bool(chosen.get("actief", True)))
                ...
                if save:
                    try:
                        ...
                        leden_upsert(payload)
                        st.success("Lid bewaard."); st.rerun()
                    except Exception as e:
                        st.error(f"Bewaren mislukt: {e}")

                # Verwijderen
                del_disabled = is_readonly() or (rid is None and not key_map.get(sel, {}).get("email") and not key_map.get(sel, {}).get("username"))
                if st.button("Verwijder lid", type="secondary", disabled=del_disabled):
                    try:
                        if rid is not None:
                            run_db(lambda c: c.table("leden").delete().eq("id", rid).execute(), what="leden delete")
                        else:
                            # Fallback op email/username indien id ontbreekt
                            km = key_map.get(sel, {})
                            if km.get("email"):
                                run_db(lambda c: c.table("leden").delete().eq("email", km["email"]).execute(), what="leden delete by email")
                            elif km.get("username"):
                                run_db(lambda c: c.table("leden").delete().eq("username", km["username"]).execute(), what="leden delete by username")
