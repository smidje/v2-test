...
def activiteit_update(act_id, titel, omschr, datum, tijd, locatie, meal_opts):
    """Update één activiteit (alleen voor beheerder)."""
    payload = {
        "titel": (titel or "").strip(),
        "omschrijving": (omschr or "").strip(),
        "datum": datum.isoformat() if hasattr(datum, "isoformat") else str(datum),
        "tijd": tijd.isoformat() if getattr(tijd, "isoformat", None) else (str(tijd) if tijd else None),
        "locatie": (locatie or "").strip() or None,
        "meal_options": meal_opts or None,
    }
    run_db(lambda c: c.table("activiteiten").update(payload).eq("id", act_id).execute(), what="activiteiten update")
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            # Beheer: activiteit bewerken (alleen admin)
            if current_role() == "admin":
                with st.expander("✏️ Beheer: activiteit bewerken", expanded=False):
                    c1, c2 = st.columns([2, 1])
                    with c1:
                        new_titel = st.text_input("Titel*", value=row.get("titel") or "", key=f"adm_t_{row['id']}")
                        new_omschr = st.text_area("Omschrijving", value=row.get("omschrijving") or "", key=f"adm_o_{row['id']}")
                    with c2:
                        import datetime as _dt
                        cur_date = pd.to_datetime(row.get("datum")).date() if row.get("datum") else _dt.date.today()
                        new_datum = st.date_input("Datum*", value=cur_date, key=f"adm_d_{row['id']}")
                        cur_time = None
                        if row.get("tijd"):
                            try:
                                cur_time = pd.to_datetime(str(row.get("tijd"))).time()
                            except Exception:
                                cur_time = None
                        new_tijd = st.time_input("Tijd (optioneel)", value=cur_time, key=f"adm_ti_{row['id']}")
                        pl = plaatsen_list()
                        cur_loc = row.get("locatie") or ""
                        new_loc = st.selectbox("Locatie", ["— kies —"] + pl, index=(pl.index(cur_loc) + 1) if cur_loc in pl else 0, key=f"adm_l_{row['id']}")

                    st.caption("Maaltijdopties (max. 3, optioneel)")
                    m1, m2, m3 = st.columns(3)
                    meal_opts = row.get("meal_options") or []
                    if isinstance(meal_opts, str):
                        try:
                            import json as _json
                            meal_opts = _json.loads(meal_opts)
                        except Exception:
                            meal_opts = []
                    meal_opts = (list(meal_opts) + [None, None, None])[:3]
                    with m1: mo1 = st.text_input("Optie 1", value=meal_opts[0] or "", key=f"adm_m1_{row['id']}")
                    with m2: mo2 = st.text_input("Optie 2", value=meal_opts[1] or "", key=f"adm_m2_{row['id']}")
                    with m3: mo3 = st.text_input("Optie 3", value=meal_opts[2] or "", key=f"adm_m3_{row['id']}")
                    new_meal_opts = [x.strip() for x in [mo1, mo2, mo3] if x and x.strip()] or None

                    if st.button("💾 Bewaar wijzigingen (beheerder)", key=f"adm_upd_{row['id']}", type="secondary"):
                        try:
                            activiteit_update(
                                row["id"],
                                new_titel,
                                ...
                                new_datum,
                                new_tijd,
                                None if not new_loc or new_loc == "— kies —" else new_loc.strip(),
                                new_meal_opts,
                            )
                            st.success("Activiteit bijgewerkt.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"Wijzigen mislukt: {e}")
