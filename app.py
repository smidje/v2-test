...
def leden_delete_by_id(lid_id):
    """Verwijder lid op basis van id."""
    return run_db(lambda c: c.table("leden").delete().eq("id", lid_id).execute(), what="leden delete")

def leden_delete_by_username(username):
    """Verwijder lid op basis van username."""
    return run_db(lambda c: c.table("leden").delete().eq("username", username).execute(), what="leden delete")
...
def page_setup_first_admin_username():
    ...
    if submitted:
        if not username or not pw1 or len(pw1) != 5 or pw1 != pw2:
            st.error("Controleer login en wachtwoord (exact 5 tekens en gelijk).")
            return
        try:
...
def page_profiel():
    ...
    if st.button("Wijzig wachtwoord"):
        if not npw1 or len(npw1) != 5 or npw1 != npw2:
            st.warning("exact 5 tekens en beide velden moeten gelijk zijn.")
        else:
            try:
...
def page_ledenbeheer():
        ...
        try:
            ...
            if not u:
                if not pw1 or len(pw1) != 5 or pw1 != pw2:
                    st.warning("Nieuwe login vereist wachtwoord (exact 5 tekens) en beide velden gelijk.")
                    return
    ...
    if ok:
        if not uname or not npw1 or len(npw1) != 5 or npw1 != npw2:
            st.warning("Controleer login en wachtwoord (exact 5 tekens, gelijk).")
        else:
            try:
    ...
    st.divider()
    st.subheader("Lid bewerken / verwijderen (beheerder)")

    df__leden = leden_list_df()
    if not df__leden.empty:
        # Selecteer een lid om te bewerken of te verwijderen
        opt = df__leden.assign(_label=lambda d: d['voornaam'].fillna('') + " " + d['achternaam'].fillna('') + " — " + d['username'].fillna(''))[['id','_label']]
        sel = st.selectbox("Kies lid", opt['id'].tolist(), format_func=dict(opt.values).__getitem__)
        row = df__leden[df__leden['id'] == sel].iloc[0]

        c1, c2 = st.columns(2)
        with c1:
            voornaam_e = st.text_input("Voornaam", value=str(row.get("voornaam") or ""))
            achternaam_e = st.text_input("Achternaam", value=str(row.get("achternaam") or ""))
            username_e = st.text_input("Username", value=str(row.get("username") or ""))
            email_e = st.text_input("E-mail", value=str(row.get("email") or ""))
        with c2:
            role_e = st.selectbox("Rol", options=["member","admin"], index=1 if str(row.get("role"))=="admin" else 0)
            brevet_e = st.selectbox("Duikbrevet", options=["(geen)"] + BREVET_CHOICES, index=0 if not row.get("duikbrevet") else (["(geen)"] + BREVET_CHOICES).index(row.get("duikbrevet")) if row.get("duikbrevet") in BREVET_CHOICES else 0)

        colA, colB = st.columns([1,1])
        with colA:
            if st.button("Wijzigingen opslaan", type="primary", key="leden_edit_save"):
                payload = {
                    "voornaam": voornaam_e.strip(),
                    "achternaam": achternaam_e.strip(),
                    "username": username_e.strip(),
                    "email": email_e.strip() or None,
                    "role": role_e,
                    "duikbrevet": None if brevet_e == "(geen)" else brevet_e,
                }
                try:
                    leden_upsert(payload)
        ...
        with colB:
            confirm = st.checkbox("Ja, ik wil dit lid definitief verwijderen")
            if st.button("Verwijder lid", type="primary", key="leden_edit_delete", disabled=not confirm):
                try:
                    leden_delete_by_id(sel)
                    st.success("Lid verwijderd.")
                    st.experimental_rerun()
                except Exception as e:
                    st.error(f"Verwijderen mislukt: {e}")
    else:
        st.info("Geen leden gevonden om te bewerken/verwijderen.")
