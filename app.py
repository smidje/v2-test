...
# --- minimale fix: fallback upsert voor 'leden' zodat 42P10 wordt opgevangen ---
def upsert_lid(payload: dict):
    tbl = supabase.table("leden")
    try:
        return upsert_lid(payload, on_conflict="email")
    except Exception as e:
        emsg = str(getattr(e, "message", e))
        if "42P10" in emsg or "no unique or exclusion constraint" in emsg:
            try:
                return upsert_lid(payload, on_conflict="username")
            except Exception as e2:
                emsg2 = str(getattr(e2, "message", e2))
                if "42P10" in emsg2 or "no unique or exclusion constraint" in emsg2:
                    return upsert_lid(payload, on_conflict="login")
                raise
        raise
# --- einde minimale fix ---
...
def page_ledenbeheer():
    ...
    if submitted:
        ...
        # - duikbrevet: alleen meesturen als kolom bestaat en waarde niet None
        try:
            _cols = set(leden_list_df().columns)
        except Exception:
            _cols = set()
        if ("duikbrevet" not in _cols) or (payload.get("duikbrevet") is None):
            payload.pop("duikbrevet", None)
