...
def leden_upsert(payload: dict):
    ...
    # Normaliseer waarden
    if "duikbrevet" in payload:
        _br = (payload.get("duikbrevet") or "").strip()
        payload["duikbrevet"] = None if _br == "" else _br

    email = (payload.get("email") or "").strip().lower()
    if not email:
        raise Exception("E-mail is verplicht voor leden_upsert.")

    # 1) Bestaat lid met dit e-mailadres?
    found = run_db(
        lambda c: c.table("leden").select("id").eq("email", email).limit(1).execute(),
        what="leden lookup by email"
    )
    rows = (found.data or [])

    if rows:
        lid_id = rows[0]["id"]
        # Update exact wat is meegegeven
        run_db(
            lambda c: c.table("leden").update(payload).eq("id", lid_id).execute(),
            what="leden update by id"
        )
    else:
        # Nog geen rij: insert
        run_db(
            lambda c: c.table("leden").insert(payload).execute(),
            what="leden insert"
        )
