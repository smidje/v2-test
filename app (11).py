...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        try:
            _ = coming_people; _ = not_coming_people
        except NameError:
            try:
                coming_people = 0
                if 's' in locals() and hasattr(s, 'empty') and not s.empty:
                    for __i, __r in s.iterrows():
                        coming_people += party_size_from_signup(__r)
                not_coming_people = int((s['status'] == 'no').sum()) if ('s' in locals() and hasattr(s, 'columns') and 'status' in s.columns) else 0
            except Exception:
