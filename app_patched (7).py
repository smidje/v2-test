...
# Tabellen & helpers
# ──────────────────────────────────────────────────────────────────────────────
BREVET_CHOICES = ["1*", "2*", "3*", "4*", "AI", "1*I", "2*I", "3*I"]
...
def get_allow_extra(activiteit_id):
    """Lees per-activiteit de instelling 'allow_extra' uit de activiteiten-tabel (optioneel)."""
    try:
        ...
        # Kolom bestaat (nog) niet of RLS/beleid verhindert lezen.
        return None
    return None
...
def set_allow_extra(activiteit_id, value: bool):
    ...
    try:
        ...
        # Toon korte hint hoe kolom aan te maken als ze ontbreekt
        st.warning("Kon 'allow_extra' niet bewaren. Bestaat de kolom op 'activiteiten'?\n"
                   "SQL: ALTER TABLE public.activiteiten ADD COLUMN allow_extra boolean DEFAULT false;")
        return False
def appbar(tag: str):
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        with st.expander(f"{titel}   ·   {badge}", expanded=False):
            ...
            # Samenvatting: aantal personen die komen en die niet komen
            try:
                coming_people = 0
                if not s.empty:
            ...
            with c_metrics1:
                st.metric("Komen", coming_people)
            with c_metrics2:
                st.metric("Niet komen", not_coming_people)
            ...
            # Overzicht: wie wat wil (eten/maaltijd)
            try:
                if not coming.empty:
                    _rows = []
                    for _idx, _r in coming.iterrows():
                        _info = parse_meal_choice_value(_r.get("meal_choice"))
                        _rows.append({
                            "Lid": _r.get("username") or _r.get("lid_id"),
                            "Eet mee": bool(_r.get("eating")) if pd.notna(_r.get("eating")) else False,
                            "Maaltijd": _info.get("primary"),
                            "Extra personen": int(_info.get("guests") or 0),
                            "Extra maaltijden": ", ".join([f"{k}: {v}" for k, v in (_info.get("guest_meals") or {}).items()]) if (_info.get("guest_meals") or {}) else ""
                        })
                    if _rows:
                        st.markdown("**Overzicht inschrijvingen (eten/maaltijd)**")
            ...
            # Laad huidige waarde uit DB (indien beschikbaar)
            _db_allow = get_allow_extra(row["id"])
            if _db_allow is not None and enable_key not in st.session_state:
            ...
            if current_role() == "admin":
                ...
                if st.button("Instelling opslaan", key=f"save_extra_{row['id']}"):
                    ok = set_allow_extra(row["id"], bool(st.session_state.get(enable_key, False)))
                    if ok:
