...
def duikers_labels() -> list[str]:
    res = run_db(lambda c: c.table("duikers").select("voornaam, achternaam, naam").execute(),
                 what="duikers select")
    rows = res.data or []
    # Fallback: als er (nog) geen duikers in de tabel staan, leid af uit leden met duikbrevet
    if not rows:
        try:
            _res = run_db(lambda c: c.table("leden").select("voornaam, achternaam, duikbrevet, actief").execute(), what="leden select (duikers fallback)")
            _rows = _res.data or []
            rows = [{"voornaam": r.get("voornaam"), "achternaam": r.get("achternaam"), "naam": None} for r in _rows if r.get("duikbrevet")]
        except Exception:
            rows = []
    out = []
    for r in rows:
...
def page_activiteiten():
    ...
    for _, row in df.sort_values(["datum", "tijd"], na_position="last").iterrows():
        ...
        titel = f"{row['titel']} — {pd.to_datetime(row['datum']).strftime(DATE_FMT)}"
        if row.get("tijd"):
            titel += f" · {row['tijd']}"
    ...
    if df2.empty:
        ...
        view["datum"] = pd.to_datetime(view["datum"]).dt.strftime(DATE_FMT)
...
def page_overzicht():
    ...
    for _, r in f2.iterrows():
        ...
        dc = dc if dc else "—"
        label = f"{r['Datum'].strftime(DATE_FMT)} · {r['Plaats']} · {r['Duiker']} · {dc}"
